...
<%
	...
	rList=twitter.getFriendsTimeline(paging);
	//msgList=(List)twitter.getHomeTimeline(paging);
...
<%					if(twiUsrId==msgUsrId)
					{%>
						<a href="#"		onclick="<portlet:namespace/>deleteStatus('<%=actTwiURL%>','<%=msgId%>','<%=pageNo%>','')">
...
<%			if(pageNo>1)
			{
%>				 
				<a href="#" onclick="<portlet:namespace/>paging('<%=actTwiURL%>','<%=pageNo%>','pre','')">Previous</a>&nbsp;
<%			}
			if((rList.size()==10)&&((twitter.getFriendsTimeline(new Paging(pageNo+1))).size()>0))
			{
%>				<a href="#" onclick="<portlet:namespace/>paging('<%=actTwiURL%>','<%=pageNo%>','next','')">Next</a>
<%			}
%>
		</center>
<%	}
